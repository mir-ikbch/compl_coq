               ...
               | Nc -> Nc)
                       (*let env = Global.env () in
										   let evd = Evd.from_env env in
											 CErrors.errorlabstrm "Ordergin Failure"
												(str "Unknown symbol: " ++ 
												Ppconstr.pr_constr_expr (Constrextern.extern_constr true env evd c1') ++
												str "\n"))*)
...
let ordered_rewrite occs weights c =
  Proofview.Goal.nf_enter { enter = fun gl ->
    ...
    let hinfo = Tacmach.New.pf_apply (find_applied_relation false Loc.ghost) gl c true in
	  let ord = try lex_pathord weights hinfo.hyp_left hinfo.hyp_right
							with _ -> Nc
    in
    if ord = Gt
